    ...
    bool include_velocity)
{
    ...
    // cout << "[mujoco_ompl_interface]: Inside Make Compound State Space" << endl;

    auto space(make_shared<ob::CompoundStateSpace>());
    // cout << "[mujoco_ompl_interface]: Created Compound Space" << endl;

    // Iterate over joints
    auto joints = getJointInfo(m);
    // cout << "[mujoco_ompl_interface]: Got Joints" << endl;
    ...
    for(const auto& joint : joints) {
        ...
        // cout << "[mujoco_ompl_interface]: Creating Sub Space Space" << endl;
        // cout << "[mujoco_ompl_interface]: Joint Type: [" << joint.type << endl;

        shared_ptr<ob::StateSpace> joint_space;
        if (joint.type != mjJNT_BALL){
            switch(joint.type) {
            case mjJNT_FREE:
                cout << "[mujoco_ompl_interface]: Joint is free. ERROR!" << endl;
            ...
            case mjJNT_BALL:
                cout << "[mujoco_ompl_interface]: Joint is a ball. ERROR!" << endl;
            ...
            case mjJNT_HINGE:
                ...
                if (joint.limited) {
                    ...
                    // cout << "[mujoco_ompl_interface]: Joint is a hinge and limited. Creating Real Vector State Space" << endl;

                    joint_space = make_shared<ob::RealVectorStateSpace>(1);
                    static_pointer_cast<ob::RealVectorStateSpace>(joint_space)
                        ->setBounds(bounds);
                } else {
                    // cout << "[mujoco_ompl_interface]: Joint is a hinge and not limited. Creating S02 State Space" << endl;
            ...
            case mjJNT_SLIDE:
                ...
                joint_space = make_shared<ob::RealVectorStateSpace>(1);
                if (joint.limited) {
                    // cout << "[mujoco_ompl_interface]: Joint is a slide and limited. Creating Real Vector State Space" << endl;
        ...
        // cout << "[mujoco_ompl_interface]: Added " << joint.name << " into subspace." << endl;
...
shared_ptr<ob::SpaceInformation> createSpaceInformationKinematic(
    const mjModel* m)
{
    // cout << "[mujoco_ompl_interface]: Inside Create Space Information Kinematic" << endl;

    auto space = makeCompoundStateSpace(m, false);
    // cout << "[mujoco_ompl_interface]: Made Compound State Space" << endl;
...
bool MujocoStateValidityChecker::isValid(const ompl::base::State *state) const {
    // cout << "[mujoco_ompl_interface]: Inside State Validity Checker" << endl;
    double contact_threshold = -0.002;
    mj_lock.lock();
    ...
    int ncon = mj->d->ncon;
    // cout << "[mujoco_ompl_interface]: Number of detected contacts: " << ncon << endl;
    ...
    if (ignored_contacts.size() == 0){
            ...
            // cout << "[mujoco_ompl_interface]: State is not Valid" << endl;
    ...
    //     cout << "Contact!. Not Valid state!!" << endl;
    // }
    mj_lock.unlock();
    return isValidState;
}
